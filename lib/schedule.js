...
export function parseWorkHours(workHours) {
  ...
  const pattern = /(\d{1,2})(?::(\d{2}))?\s*(ص|م|am|pm)?\s*(?:-|–|—|إلى|الى|to)\s*(\d{1,2})(?::(\d{2}))?\s*(ص|م|am|pm)?/gi;
  const windows = [];
  let match;
...
/* All slots for one date. `booked` is a Set of "HH:MM" already taken;
   `now` lets same-day slots in the past be disabled. */
