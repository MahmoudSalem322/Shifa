...
const ARABIC_ALIASES = Object.keys(GOV_ALIASES).sort((a, b) => b.length - a.length);
...
export const geo = {
  normalize(value) {
    ...
    /* Free-text addresses: match the governorate name they mention, longest
       alias first so "شمال غزة" is not read as "غزة". */
    for (const alias of ARABIC_ALIASES) {
      if (/[؀-ۿ]/.test(alias) && raw.includes(alias)) return GOV_ALIASES[alias];
    }
    return '';
  },
  label(value) {
