...
import { COLLECTION, assertPending, isReviewer, publicDonation } from './donations';
...
export function reviewHandler(decision) {
  return handle(async (request, { params }) => {
    ...
    if (!isReviewer(user)) throw new HttpError(403, 'مراجعة التبرعات متاحة للصيدليات والمراكز الصحية فقط.');
