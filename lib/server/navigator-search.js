...
async function fetchList(user, path, params = {}) {
  try {
    const response = await backend(user, 'GET', path + query(params));
...
async function searchMedicines(user, parsed) {
  ...
  const medicines = raw.map(normalizeMedicine).filter(Boolean)
    .filter((m) => fold(m.name).includes(wanted) || fold(m.scientificName).includes(wanted) || wanted.includes(fold(m.name)));
