...
export default function FacilityEquipmentRequestsPage() {
  ...
  const [requests, setRequests] = useState([
    { id: 1, equipment: 'جهاز ضغط دم', quantity: 5, status: 'approved', date: '2023-11-21' },
    { id: 2, equipment: 'كرسي متحرك', quantity: 2, status: 'pending', date: '2023-11-19' },
    { id: 3, equipment: 'أسطوانة أكسجين', quantity: 10, status: 'pending', date: '2023-11-18' },
  ]);
  ...
  const handleAdd = (req) => {
    ...
    setAdding(false);
    api.inbox.notifyAdmin(`تم طلب ${req.quantity} وحدة من ${req.equipment}`).catch(() => {});
  };

  return (
    <>
      <PageHeader title="طلبات المعدات" subtitle="متابعة وتحديث طلبات المعدات والأجهزة الطبية" />
      <PageBody>
        <RoleGate allow={['Hospital']} message="هذه الصفحة مخصصة للمراكز الصحية والمستشفيات فقط.">
          <Card>
            <CardTitle icon="medical_services">قائمة الطلبات</CardTitle>
              ...
              {requests.map((req) => (
                <div key={req.id} className="flex flex-wrap items-center justify-between gap-space-sm p-space-sm rounded-xl bg-surface-subtle">
                  <div className="flex flex-col">
                    <span className="font-headline-sm text-headline-sm text-text-heading">{req.equipment}</span>
                    <span className="font-body-sm text-body-sm text-text-muted">الكمية: {req.quantity} قطعة</span>
                  </div>
            ...
            <div className="mt-2 flex justify-end border-t border-border-subtle pt-space-md">
              <Button icon="add" onClick={() => setAdding(true)}>طلب معدات جديدة</Button>
            </div>
          </Card>
          <AddEquipmentModal open={adding} onClose={() => setAdding(false)} onAdded={handleAdd} />
        </RoleGate>
      </PageBody>
    </>
  );
...
function AddEquipmentModal({ open, onClose, onAdded }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    if (!name.trim()) return;
    onAdded({ equipment: name.trim(), quantity: Number(quantity) });
  };

  return (
    <Modal open={open} onClose={onClose} title="طلب معدات جديدة">
      <form className="flex flex-col gap-space-md" onSubmit={submit}>
        <Field label="اسم الجهاز / المعدة" htmlFor="req-eq" required>
          <input id="req-eq" className={inputClass} placeholder="مثال: جهاز ضغط دم، كرسي متحرك..." value={name} onChange={(e) => setName(e.target.value)} required />
        </Field>
        <Field label="الكمية المطلوبة (قطعة)" htmlFor="req-qty" required>
